...
class mymap {
 private:
     ...
     */
    NODE* _rebalance(vector<NODE*> v, int left, int right) {
        if (left > right) {
            return nullptr;
        }
        ...
        // left subtree
        curr->left = _rebalance(v, left, mid - 1);
        // right subtree
        curr->right = _rebalance(v, mid + 1, right);
        return curr;
    }
    ...
    void rebalance(bool violation, NODE*& violatorParent, NODE* violator) {
        if (violation) {
            ...
            NODE* fixedSubtree = _rebalance(subtree, 0, subtree.size() - 1);
            ...
            if (violator == root) {
                ...
                rightMost->right = violatorParent; // thread last node
